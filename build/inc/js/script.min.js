...
}),{top:offset.top-parentOffset.top-jQuery.css(elem,"marginTop",!0),left:offset.left-parentOffset.left-jQuery.css(elem,"marginLeft",!0)}}},offsetParent:function(){return this.map(function(){for(var offsetParent=this.offsetParent;offsetParent&&"static"===jQuery.css(offsetParent,"position");)offsetParent=offsetParent.offsetParent;return offsetParent||documentElement})}}),jQuery.each({scrollLeft:"pageXOffset",scrollTop:"pageYOffset"},function(method,prop){var top="pageYOffset"===prop;jQuery.fn[method]=function(val){return access(this,function(elem,method,val){var win=getWindow(elem);return void 0===val?win?win[prop]:elem[method]:void(win?win.scrollTo(top?win.pageXOffset:val,top?val:win.pageYOffset):elem[method]=val)},method,val,arguments.length)}}),jQuery.each(["top","left"],function(i,prop){jQuery.cssHooks[prop]=addGetHookIf(support.pixelPosition,function(elem,computed){if(computed)return computed=curCSS(elem,prop),rnumnonpx.test(computed)?jQuery(elem).position()[prop]+"px":computed})}),jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:"inner"+name,content:type,"":"outer"+name},function(defaultExtra,funcName){jQuery.fn[funcName]=function(margin,value){var chainable=arguments.length&&(defaultExtra||"boolean"!=typeof margin),extra=defaultExtra||(margin===!0||value===!0?"margin":"border");return access(this,function(elem,type,value){var doc;return jQuery.isWindow(elem)?0===funcName.indexOf("outer")?elem["inner"+name]:elem.document.documentElement["client"+name]:9===elem.nodeType?(doc=elem.documentElement,Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])):void 0===value?jQuery.css(elem,type,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:void 0,chainable)}})}),jQuery.fn.extend({bind:function(types,data,fn){return this.on(types,null,data,fn)},unbind:function(types,fn){return this.off(types,null,fn)},delegate:function(selector,types,data,fn){return this.on(types,selector,data,fn)},undelegate:function(selector,types,fn){return 1===arguments.length?this.off(selector,"**"):this.off(types,selector||"**",fn)}}),jQuery.parseJSON=JSON.parse,"function"==typeof define&&define.amd&&define("jquery",[],function(){return jQuery});var _jQuery=window.jQuery,_$=window.$;return jQuery.noConflict=function(deep){return window.$===jQuery&&(window.$=_$),deep&&window.jQuery===jQuery&&(window.jQuery=_jQuery),jQuery},noGlobal||(window.jQuery=window.$=jQuery),jQuery}),jQuery(function($){$(".checkbox").change(function(){$(".switchthree").toggleClass("switchthreeOn")}),$("#contact").submit(function(){var company=$("#company").val(),name=$("#name").val(),email=$("#email").val(),phone=$("#phone").val();if(""==company)return $("#company").addClass("hightlight"),!1;if($("#company").removeClass("hightlight"),""==name)return $("#name").addClass("hightlight"),!1;$("#name").removeClass("hightlight");var atpos=email.indexOf("@"),dotpos=email.lastIndexOf(".");return atpos<1||dotpos<atpos+2||dotpos+2>=email.length?($("#email").addClass("hightlight"),!1):($("#email").removeClass("hightlight"),""==phone?($("#phone").addClass("hightlight"),!1):($("#phone").removeClass("hightlight"),$("input[type=checkbox]").each(function(){if(!$(this).is(":checked"))return alert("Please you should accept the terms to proceed the process of sending. Thank you."),!1;$(".spa-loader").addClass("spa-loader-open"),$.ajax({type:"POST",url:"email.php",data:$("#contact").serialize(),dataType:"json",complete:function(r){document.getElementById("contact").reset(),$(".msg-contact").css("display","block"),$(".switchthree").addClass("switchthreeOn"),$(".spa-loader").removeClass("spa-loader-open"),ga("send","event","EDM","Strategy One")}})}),!1))}),$('a[href*="#"]:not([href="#"])').on("click",function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var target=$(this.hash);if(target=target.length?target:$("[name="+this.hash.slice(1)+"]"),target.length)return $("html,body").animate({scrollTop:target.offset().top},600),!1}})});